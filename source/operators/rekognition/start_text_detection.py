...
###############################################################################
# PURPOSE:
#   Lambda function to perform Rekognition text detection on image and video files
#
###############################################################################
...
def start_text_detection(bucket, key):
    try:
        ...
        print('Job Id (text_detection): ' + response['JobId'])
...
def lambda_handler(event, context):
    try:
        ...
        output_object.update_workflow_status("Error")
        output_object.add_workflow_metadata(TextDetectionError="No valid inputs")
        raise MasExecutionError(output_object.return_output_object())
    ...
    if file_type in valid_image_types:
        ...
        if "Status" not in metadata_upload:
            ...
            output_object.add_workflow_metadata(
                TextDetectionError="Unable to upload metadata for asset: {asset}".format(asset=asset_id))
            raise MasExecutionError(output_object.return_output_object())
        else:
        ...
        job_id = start_text_detection(s3bucket, urllib.parse.unquote_plus(s3key))
        ...
        output_object.update_workflow_status("Error")
        output_object.add_workflow_metadata(TextDetectionError="Not a valid file type")
        raise MasExecutionError(output_object.return_output_object())
