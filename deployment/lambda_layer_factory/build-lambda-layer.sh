...
if [ -n "$S3_FQDN" ]; then
    ...
    aws lambda list-layer-versions --layer-name $LAYER_NAME_39 | grep "\"LayerVersions\": \["
    if [ $? -eq 0 ]; then
        echo "WARNING: AWS Layer with name $LAYER_NAME_39 already exists."
        read -r -p "Are you sure you want to overwrite $LAYER_NAME_39? [y/N] " response
        if [[ "$response" =~ ^([yY][eE][sS]|[yY])+$ ]]
        then
            aws s3 cp lambda_layer-python3.9.zip s3://"$LAMBDA_LAYERS_BUCKET"
            aws lambda publish-layer-version --layer-name $LAYER_NAME_39 --content S3Bucket="$LAMBDA_LAYERS_BUCKET",S3Key=lambda_layer-python3.9.zip --compatible-runtimes python3.9
            aws s3 rm s3://"$LAMBDA_LAYERS_BUCKET"/lambda_layer-python3.9.zip
            arn39=$(aws lambda list-layer-versions --layer-name $LAYER_NAME_39 --output text --query 'LayerVersions[0].LayerVersionArn')
        fi
    fi
