...
def test_upload_should_fail(test_client):
    ...
    assert response.json_body['Message'] == "Unable to generate pre-signed S3 URL for uploading media: 'S3Bucket'"
    print ('Pass')
...
def test_download_fail(test_client):
    ...
    assert response.json_body['Message'] == "Unable to generate pre-signed S3 URL for downloading media: 'S3Bucket'"
    print ('Pass')
...
def test_create_asset_input_error(test_client, s3_client_stub):
    ...
    assert formatted_response['Message'] == "Missing required inputs for asset creation: 'S3Key'"
    print("Pass")
