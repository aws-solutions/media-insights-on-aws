...
![MI logo](docs/assets/images/MI_logo.png)

Media Insights on AWS (MI) is a development framework for building serverless applications that process video, images, audio, and text  on AWS. MI takes care of workflow orchestration and data persistence so that you can focus on workflow development. By addressing the concerns of running workflows, MI empowers you to build applications faster with the benefit of inheriting a pre-built and robust back end.

MI has been successfully used in a variety of scenarios, such as:
...
This repository contains the MI back-end framework. Users interact with MI through REST APIs or by invoking MI Lambda functions directly. You will not find a graphical user interface (GUI) in this repository, but a reference application for MI that includes a GUI is in the [Content Analysis](https://github.com/aws-solutions/aws-content-analysis) repository. 

# Install

You can deploy MI in your AWS account with the following Cloud Formation templates. **The Cloud Formation stack name must be 12 or fewer characters long**.
...
Run the following commands to build and deploy MI from scratch. Be sure to define values for `MI_STACK_NAME` and `REGION` first.
...
If you're building applications on MI then you will need to understand the following resources in the **Outputs** tab of the Cloud Formation stack:

* **DataplaneApiEndpoint** is the endpoint for accessing dataplane APIs to create, update, delete and retrieve media assets
* **DataplaneBucket** is the S3 bucket used to store derived media (_derived assets_) and raw analysis metadata created by MI workflows.
* **WorkflowApiEndpoint** is the endpoint for accessing the Workflow APIs to create, update, delete and execute MI workflows.
* **WorkflowCustomResourceArn** is the custom resource that can be used to create MI workflows in CloudFormation scripts

# Cost

You are responsible for the cost of the AWS services used while running this solution. As of April, 2021 the cost for running this solution with the default settings in the us-east-1 (N. Virginia) region is approximately **$24 per month without free tiers**, or **$13 per month with free tiers** for 100 workflow runs. Most MI use cases are covered by the free tier for all AWS services except Amazon Kinesis and AWS Lambda. The costs for the Amazon Kinesis data stream ($12.56/mo) and the Workflow Scheduler lambda ($3.73/mo) will remain relatively unchanged, regardless of how many workflows execute.
...
The Cloud Formation stack name for MI must be 12 or fewer characters long. This will ensure all the resources in MI stack remain under the maximum length allowed by Cloud Formation.

MI does not inherently limit media attributes such as file size or video duration. Those limitations depend on the services used in user-defined workflows. For example, if an MI workflow uses Amazon Rekognition, then that workflow will be subject to the limitations listed in the [guidelines and quotas for Amazon Rekognition](https://docs.aws.amazon.com/rekognition/latest/dg/limits.html). For those who use  the Amazon Rekognition service within MI workflows, be aware about [use cases that involve public safety](https://docs.aws.amazon.com/rekognition/latest/dg/considerations-public-safety-use-cases.html) and the general [AWS Service Terms](https://aws.amazon.com/service-terms/).

# Architecture Overview

Deploying MI builds the following environment in the AWS Cloud:
...
5. **Resource:** AWS Lambda functions for using the following commonly used services in MI workflows: Amazon Rekognition, Amazon Comprehend, Amazon Translate, Amazon Transcribe, Amazon Polly, and AWS Elemental MediaConvert
...
* **Control plane:** The control plane includes the workflow API and state machines for workflows. Workflow state machines are composed of operators from the MI operator library. When operators within the state machine are run, they interact with the MI data plane to store and retrieve derived asset and metadata generated from the workflow.
...
* **Operators:** Operators are generated state machines that call AWS Lambda functions to perform media analysis or media transformation tasks. Users can define custom operators, but the MI operator library includes the following pre-built operators:
...
You can deploy MI in your AWS account with the [one-click deploy buttons](#install) shown above. 
...
| `ExternalBucketArn` | `` | The ARN for Amazon S3 resources that exist outside the MI stack which may need to be used as inputs to MI workflows. The ARN must be a valid [Amazon S3 ARN](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-arn-format.html) and must reference the same AWS account that is used for the MI stack. By default, ExternalBucketArn will be blank, meaning workflows will only be able to input media files from the MI data plane bucket. |

# Developers

Join our Gitter chat at [https://gitter.im/awslabs/aws-media-insights-engine](https://gitter.im/awslabs/aws-media-insights-engine)! This public chat forum was created to foster communication between MI developers worldwide.
...
MI uses AWS_IAM to authorize REST API requests. The following screenshot shows how to test authentication to the MI API using Postman. Be sure to specify the `AccessKey` and `SecretKey` for your own AWS environment.
...
Amazon Macie can help you discover and protect sensitive data in AWS. If your MI use-case generates and stores sensitive data to Amazon S3, we recommend that you enable Amazon Macie on the MI Dataplane Amazon S3 bucket.

# Uninstall

To uninstall MI, delete the CloudFormation stack, as described below. This will delete all the resources created by the MI template except the `Dataplane` and the `DataplaneLogs` S3 buckets. These two buckets are retained when the solution stack is deleted in order to help prevent accidental data loss. You can use either the AWS Management Console or the AWS Command Line Interface (AWS CLI) to empty, then delete those S3 buckets after deleting the CloudFormation stack.

### Option 1: Uninstall using the AWS Management Console
1. Sign in to the AWS CloudFormation console.
2. Select the MI stack.
3. Choose Delete.
...
### Deleting MI data buckets
MI creates two S3 buckets that are not automatically deleted. To delete these buckets, use the steps below.
...
MI sends anonymous operational metrics to AWS when it is deployed. We use this data to help improve the quality of the MI solution. It consists of the following information:

* **Solution ID:** The MI solution identifier (`SO0163`)
* **Unique ID (UUID):** Randomly generated, unique identifier for each Media Insights on AWS deployment
* **Timestamp:** Date and time of the MI stack deployment
...
The [MI logo](docs/assets/images/MI_logo.png) features a clapperboard representing *multimedia*, centered inside a crosshair representing *under scrutiny*. 
