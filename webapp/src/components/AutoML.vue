  ...
  export default {
    ...
    methods: {
      ...
      drawBoxes: function(boxMap) {
        ...
        // labels will be cleared when a before a new label is drawn
        // or after 2 seconds if no new label is available in that time
        this.erase_on_iteration = 40;
        var i = 0;
        this.canvasRefreshInterval = setInterval(function () {
          i++;
          // erase old bounding boxes
          if (!this.player.paused() && i % this.erase_on_iteration === 0) {
            i=0;
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            ctx.beginPath();
            ctx.strokeStyle = "red";
          ...
          // for (i=0; i<10; i++) {
          // var player_timestamp = Math.floor(this.player.currentTime()*10.0 + i)
          var player_timestamp = Math.round(this.player.currentTime()*10.0);
          // console.log(player_timestamp)
          // If we have a box for the player's timestamp...
          if (boxMap.has(player_timestamp)) {
            console.log("box found at time " + player_timestamp)
            i=0
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            ctx.beginPath();
            ctx.strokeStyle = "red";
            ...
            instance_list.forEach( i => {
              ...
              // Determine which box has the highest confidence
              // var highconfidence = Math.max.apply(Math, boxes.map(function(o) { return o.confidence; }));
              // var drawMe = boxes.find(box => box.confidence === highconfidence.toString())
